...
def discriminator_loss(logits_real, logits_fake):
    ...
    ####################################
    N = logits_real.shape
    N2 = logits_fake.shape
    real_loss = bce_loss(logits_real, torch.ones(N2).cuda().detach(), reduction = 'mean')
    fake_loss = bce_loss(1 - logits_fake, torch.ones(N2).cuda().detach(), reduction = 'mean')
...
def generator_loss(logits_fake):
    ...
    ##########       END      ##########
    N = logits_fake.shape
    return bce_loss(logits_fake, torch.ones(N).cuda().detach(), reduction = 'mean')
