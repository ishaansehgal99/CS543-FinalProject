...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        norm = torch.norm((torch.mv(torch.t(w.view(h, -1).data), u.data)))
        v = torch.div(torch.mv(torch.t(w.view(h, -1).data), u.data), norm)

        norm = torch.norm(torch.mv(w.view(h, -1).data, v.data))
        u = torch.div(torch.mv(w.view(h, -1).data, v.data), norm)

        last = u.dot(w.view(h, -1).mv(v))
        wsn = torch.div(w, last.expand_as(w))

        setattr(self.module, self.name, wsn)
