...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        for _ in range(self.power_iterations):
            v.data = l2normalize(torch.mv(torch.t(w.view(w.data.shape[0], -1).data), u.data))
            u.data = l2normalize(torch.mv(w.view(w.data.shape[0], -1).data, v.data))

        Wsm = u.dot(w.view(w.data.shape[0], -1).mv(v))
        setattr(self.module, self.name, w / Wsm.expand_as(w))
